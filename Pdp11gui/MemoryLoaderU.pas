...
// alle bytes der memorycellgroup in die Datei
...
//                [stream_byte_idx, state, b, sum, block_byte_idx]);
 //       OutputDebugString('[0x%0.4x] state=%d ch=0x%0.2x sum=0x%0.2x block_byte_idx=%d',
//                [stream_byte_idx, state, b, sum, block_byte_idx]);
          inc(stream_byte_idx) ;
          case (state) of 
            ...
            5: begin // read address high
              ...
              if (block_byte_idx > block_byte_size) then begin 
                ...
                if block_databyte_count = 0 then begin
                  ...
                  Log('Starting data block with addr = %s, block_byte_size=%d, datawordcount=%d', [ 
                          Dword2OctalStr(address,16), block_byte_size, block_databyte_count]);
...
procedure TMemoryLoader_StandardAbsolutePapertape.Save(memorycellgroup: TMemoryCellgroup) ; 

  var 
    stream: TFileStream ; 
    // output address sub range of memorycellgroup as on e paertape record
    // Again the foramt:
    //  block := <stuff bytes> <0x01> <0x00> <blocksize_low> <blocksize_high>
    //          <startaddr_low> <startaddr_high>
    //        ( <blocksize> - 6) / 2 x <data_low> <data_high>
    //         <chksum>
    //   <blocksize> includes everything from <0x01> to last data byte
    //   everything from 0x01 to <chksum> goes into checksum, result must be 0

  procedure WriteBlock(startaddr: dword; data_mcg: TMemoryCellgroup ; data_startidx, data_endidx: dword) ; 

    var 
      TheByteSum: dword ; // global sum of written bytes

    procedure WriteB(b: byte) ; 
      begin 
        stream.Write(b,1) ; 
        TheByteSum := TheByteSum + b ; 
      end; 

    // write a 2 byte word with lsb first
    procedure WriteWordLH(w: word) ; 
      begin 
        WriteB(w and $ff) ; // lsb first
        WriteB(w shr 8) ; // then msb
      end; 
    ...
    var 
      ...
      blocksize: word ; 
      chksum: dword ; 
    begin { "procedure WriteBlock" } 
      // blocksize = header + blocksize + startaddr + data
      if data_endidx < data_startidx then 
        blocksize := 0 // no data given
      else 
        blocksize := 2 * (data_endidx - data_startidx) + 2 ; // in bytes. addr range is words
      blocksize := blocksize + 6 ; 
      TheByteSum := 0 ; // init output

      // header
      WriteB(1) ; WriteB(0) ;
      //  <blocksize_low> <blocksize_high>
      WriteWordLH(blocksize);
      // <startaddr_low> <startaddr_high>
      WriteWordLH(startaddr);

      for i := data_startidx to data_endidx do
        WriteWordLH(data_mcg.Cell(i).edit_value);

      // checksum: sum of all bytes must be 0
      chksum := 256 - (TheByteSum and $ff) ;
      WriteB(chksum and $ff ) ;

      // stuff bytes: Sometimes I found 4x 00 ... makes a visible gap on paper tape
      WriteB(0) ; WriteB(0) ; WriteB(0) ; WriteB(0) ;
  ...
  var 
    ...
    curaddr: dword ; 
    blockstartidx: dword ; 
    blockstartaddr, blockendaddr: word ; 
    val: word ; 
    i: integer ; 
  begin { "procedure TMemoryLoader_StandardAbsolutePapertape.Save" } 
    assert(memorycellgroup.mat > matAnyPhysical) ; 

    fname := getFile(0).filename ; 
    Log('TMemoryLoader_StandardAbsolutePapertape.Save(): writing file %s', [fname]) ; 
    ...
    try 
      ...
      // Ausgabe: "addr: value0 value1 .. value7"
      // new line if addr mod 8 = 0
      blockstartaddr := memorycellgroup.Cell(0).addr.val ; 
      blockendaddr := blockstartaddr ; // inclusive

      if (blockstartaddr > $fffe) then 
        raise Exception.CreateFmt('Start address %s too large for 16 bit', 
                [Dword2OctalStr(blockstartaddr)]) ; 
      if (blockendaddr > $fffe) then 
        raise Exception.CreateFmt('End address %s too large for 16 bit', 
                [Dword2OctalStr(blockendaddr)]) ; 
      // now addresses are guranteed to be 16 bit

      blockstartidx := 0 ; 
      for i := 1 to wordcount - 1 do begin 
        curaddr := memorycellgroup.Cell(i).addr.val ; 
        val := memorycellgroup.Cell(i).edit_value ; 
        if val <> MEMORYCELL_ILLEGALVAL then begin 
          if curaddr = (blockendaddr+2) then 
            blockendaddr := curaddr // curaddr still in block
          else begin 
            // Block end: write current block
            WriteBlock(blockstartaddr, memorycellgroup, blockstartidx, i-1) ; 
            // start next block
            blockstartaddr := curaddr ; 
            blockendaddr := curaddr ; 
            blockstartidx := i ; 
          end ; 
        end; 
      end { "for i" } ; 
      // last block still open
      WriteBlock(blockstartaddr, memorycellgroup, blockstartidx, wordcount-1) ; 

      // Startaddresse als block der Länge 0 (ohne data)
      // addresse "1" = not valid, wird trotzdem geschrieben
      if (EntryAddress.val <> MEMORYCELL_ILLEGALVAL) then begin 
        WriteBlock(EntryAddress.val, nil, maxint, 0) ; 
      end ; 

    finally 
      stream.Free ; 
    end{ "try" } ; 
  end { "procedure TMemoryLoader_StandardAbsolutePapertape.Save" } ; 
